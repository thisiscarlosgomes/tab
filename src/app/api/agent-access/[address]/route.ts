...
async function maybeConfigureAgentSigner({
  ...
  const signerId = process.env.PRIVY_AGENT_SIGNER_ID?.trim();
  const policyIds = parsePolicyIds(process.env.PRIVY_AGENT_POLICY_IDS);

  if (!signerId && policyIds.length === 0) return;
  if (!userJwt) {
  ...
  await privy.wallets().update(walletId, {
    ...(signerId
      ...
      : {}),
    ...(policyIds.length > 0 ? { policy_ids: policyIds } : {}),
