...
import { fetchTwitterFollowersForUser } from "@/lib/twitter";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const profiles = await fetchTwitterFollowersForUser({
    ...
    return Response.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to fetch Twitter followers",
      },
      { status: 400 }
    );
  }
