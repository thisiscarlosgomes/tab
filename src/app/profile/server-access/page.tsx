...
export default function AgentAccessPage() {
  ...
  const isSelectedWalletDelegated = Boolean(
    normalizedAgentWalletAddress &&
      user?.linkedAccounts?.some(
        (account) =>
          account.type === "wallet" &&
          account.delegated === true &&
          normalizeAddress(account.address) === normalizedAgentWalletAddress
      )
  );
  ...
  const hasDelegation = Boolean(
    isSelectedWalletDelegated && !isRevoked
  );
  ...
  const loadAgentAccess = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/server-access/me", {
        headers: authHeaders,
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error ?? "Failed to load server access");
  ...
  const savePolicy = async (
    ...
    try {
      const res = await fetch(`/api/server-access/${targetAddress}`, {
        method: "PUT",
        headers: {
      ...
      toast.success("Server policy updated");
  ...
  const enableAgentAccess = async () => {
    ...
    try {
      ...
      const delegateRes = await fetch(
        `/api/server-access/${normalizedAgentWalletAddress}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      toast.success("Server access enabled");
      await loadAgentAccess();
    } catch (error) {
  ...
  const delegateWalletOnly = async () => {
    ...
    try {
      setSavingAction("enable");
      await delegateWallet({ address: agentWalletAddress, chainType: "ethereum" });
      toast.success("Wallet delegation approved");
      await loadAgentAccess();
    } catch (error) {
    ...
    return (
      <div className="min-h-screen p-4 pt-[calc(5rem+env(safe-area-inset-top))]">
        <div className="max-w-md mx-auto text-white/70">
          Sign in to manage server access.
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-md mx-auto space-y-3">
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-white/50 mt-1">
                Enable server-side wallet delegation for Tab assistant
              </p>
            </div>
          </div>
        </button>
        ...
        <div className="border border-white/10 rounded-xl p-4 space-y-1">
          ...
          <p className="text-white/50 text-sm">
            Tab assistant executes from this delegated wallet with strict limits.
          </p>
          ...
          {!hasLinkedFarcaster && (
            <p className="text-yellow-300/80 text-sm">
              Link Farcaster first. Split settlement needs social identity.
            </p>
          )}
        </div>
        ...
        <div className="space-y-2">
          {!isSelectedWalletDelegated && (
            <Button
              onClick={delegateWalletOnly}
              disabled={loading || saving || !agentWalletAddress}
          ...
          {!hasLinkedFarcaster && (
            <p className="text-xs text-yellow-300/80 px-1">
              Link Farcaster to allow split settlement.
            </p>
          )}
          ...
          <div className="max-w-md mx-auto space-y-4">
            <ResponsiveDialogHeader className="pt-0">
              ...
              <ResponsiveDialogDescription className="sr-only">
                Instructions for enabling server-side wallet delegation for assistant writes.
              </ResponsiveDialogDescription>
            </ResponsiveDialogHeader>

            <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3 space-y-2">
              <p className="text-sm text-white/90">
                <span className="text-white/50">1.</span> Enable delegation for your embedded wallet
              </p>
              <p className="text-sm text-white/90">
                <span className="text-white/50">2.</span> Save your spending guardrails
              </p>
              <p className="text-sm text-white/90">
                <span className="text-white/50">3.</span> Return to assistant and confirm a write action
              </p>
            </div>
            ...
            <p className="text-xs text-white/45">
              Assistant writes only from your delegated wallet and within your guardrails.
            </p>
