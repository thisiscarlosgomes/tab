export const getTokenPrices = async (): Promise<Record<string, number>> => {
  ...
  try {
    const res = await fetch(
      "https://api.coingecko.com/api/v3/simple/price?ids=ethereum,wrapped-ethereum,usd-coin,euro-coin&vs_currencies=usd"
    );
    const data = await res.json();

    if (data?.ethereum?.usd) prices["ETH"] = data.ethereum.usd;
    if (data?.["wrapped-ethereum"]?.usd) prices["WETH"] = data["wrapped-ethereum"].usd;
